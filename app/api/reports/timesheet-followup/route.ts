...
import { renderFollowUpSummaryEmail, renderPersonalShortfallEmail } from "@/lib/emailTemplates/timesheetReminders";
import { isResendConfigured, sendEmail } from "@/lib/resend";

// Vercel Cron only issues GET requests -- see vercel.json for the schedule
// (Monday 02:00 UTC = Monday 12:00 AEST, QLD has no DST). The midday round:
// "Timesheet Overview" -- a whole-FY shortfall email to whoever's still
// short (a harder-hitting follow-up than the morning nudges in
// timesheet-reminder/timesheet-reminder-2, since it surfaces every
// incomplete week this FY, not just last week) -- and the Partner-facing
// firm-wide summary, sent regardless of whether anyone's short (so "all
// clear" is visible too, same convention as the Monday Report's combined
// report).
...
export async function GET(request: NextRequest) {
  ...
  const allResults = [...shortfallResults, ...summaryResults];
