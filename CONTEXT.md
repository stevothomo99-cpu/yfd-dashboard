...
Every `staff.included` staff member gets a personal weekly email (**"Workflow Update"**); every included Partner additionally gets a combined firm-wide one (**"Overdue Summary"**). **These two now fire from two separate routes at two separate times** (split in v5.2 per the practice's requested schedule — see Settings → Email Schedule, and §6.8 for the full picture including the timesheet reminders): the Partner's Overdue Summary a night ahead of everyone else's Workflow Update, not at the same moment. Both authenticated via `CRON_SECRET` (the header Vercel Cron sends automatically once that env var is set). `RESEND_API_KEY`/`RESEND_FROM_EMAIL`/`CRON_SECRET` are now set in Vercel (same Resend account already confirmed sending the BAS approval emails, §6.5) — these two crons should now actually send once deployed, though each hasn't yet been individually confirmed end-to-end in production.
...
- **Resend account/domain + `RESEND_API_KEY`/`RESEND_FROM_EMAIL`/`CRON_SECRET` are now set in Vercel** — BAS Status approval emails (§6.5) are confirmed sending in production (landed in Junk initially, since resolved by marking not-junk); the Monday Report (§6.4) and timesheet reminder emails (§6.8) should now also send once deployed, but haven't each been individually confirmed end-to-end yet — worth a first-Monday check.
