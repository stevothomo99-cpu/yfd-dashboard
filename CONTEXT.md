...
v3.0 described the XPM-native practice-management system replacing Karbon. v4.0 keeps that architecture and records a large round of correctness work on top of it: the timesheet figures were wrong in three independent ways (§4.1, §6.1), a client's Manager was being inferred rather than read (§6), and the XPM client silently dropped data under rate limiting (§4.1). v4.1 adds a fourth timesheet correction — a billable-share denominator that omitted a bucket, and unlogged hours that were counted nowhere (§6.1). v4.2 removes the last of Karbon from Settings and makes the Included staff toggles actually take effect (§6.3). v4.3 moves tasks/workflow off jobs and onto clients entirely (§6) — the same job-vs-client confusion already fixed once on Manager allocations, this time in the tasks table's own foreign key. v5.0 is one long session's worth of feature work on top of that foundation: a new BAS approval-pipeline page (§6.5), a weekly Monday Report email (§6.4), a task completion audit trail (§6.6), a `/my-work` and `/timesheets` round of polish, and an in-progress `/team`+`/leaderboard` merge that three earlier attempts had failed to land. v5.0's own "Pick up here" note was closed out in v5.1: the `/team`+`/leaderboard` merge is done — see §6.7. v5.2 adds a full weekly email schedule (§6.4/§6.8/§6.9) — a Sunday firm-wide overdue summary, Monday-morning per-staff workflow update, two timesheet submission reminders, and a midday round of a Partner summary plus a personal whole-FY shortfall overview, all recorded as a standing reference at Settings → Email Schedule. v5.3 adds a Daily Digest (§6.10) — a lighter overdue+due-today email firing every morning except Monday — and, more importantly, records that none of this schedule is actually delivering yet: a live production check on 4 Sep 2026 found every `/api/reports/*` cron firing on schedule but returning `401` from its own `CRON_SECRET` check, meaning that env var isn't actually set correctly in Vercel's Production environment (§6.4) — check this before assuming any of §6.4/§6.8/§6.9/§6.10 is live. v5.4 adds outstanding Dashboard To-Do items to both the Workflow Update and Daily Digest (§6.4/§6.10, via `lib/todos.ts`'s new `listOutstandingTodoLines`) — the one piece of the practice's spec that had no email presence at all — and retimes the Overdue Summary from Sunday 20:00 AEST to the actually-requested Sunday 12:00pm (midday) AEST. v5.5 (§4.8) makes the To-Do modal show the original forwarded email's body (previously only the subject was visible), and adds an "Assign to" picker with a full reassignment loop — migration 026's `assigned_by_staff_id` tracks who last reassigned a to-do (distinct from `created_by_email`/`created_by_name`, the original forwarder), the new owner gets emailed, and whoever assigned it gets emailed back once it's marked done or discarded. **v5.6 turns the static Email Schedule page into an interactive Settings → Notifications page** (§6.9, `lib/notificationRegistry.ts`) — a real "View template" preview per notification (built from live current data) and a recipients checklist (UI only, not yet persisted) — and **starts drafting a second, genuinely different category: client-facing notifications** (§6.11), beginning with "BAS Workflow". That's UI/template only so far — a checkbox + schedule picker on the task modal, a draft email template, all clearly flagged as not wired to anything real yet, since the data model has no client contact email to send to and no per-task schedule engine.
...
2. **Inbound webhook** (`/api/email/inbound`) — powers the "email-to-todo" feature. Needs `RESEND_INBOUND_API_KEY` — a separate env var from `RESEND_API_KEY` above for historical reasons (kept as two names in case outbound and inbound ever need to split across accounts again), but currently the **same account's key** as `RESEND_API_KEY`. Forward an email to `TODO_INBOUND_EMAIL` — `todo@dashboard.yourfinancedept.com.au`, a dedicated **subdomain**, not the bare domain, so Resend's inbound MX records don't override `yourfinancedept.com.au`'s real Microsoft 365 mail — and it creates a lightweight **To-Do item** (`todo_items` table — see §6) shown on `/dashboard`.
   ...
   - **Original email body is viewable, not just the subject** (v5.5): the populate/edit modal has a "Show original email" toggle revealing `todo.body` in a scrollable box -- previously only the subject line was shown anywhere, so the actual forwarded content was invisible without going back to your inbox.
   - **Reassignment via an "Assign to" picker** (v5.5, migration 026's `assigned_by_staff_id`): both the populate and edit modals let you hand a to-do to a different staff member (unfiltered staff list, so a to-do's current owner is always a valid option even if since excluded from reporting). Reassigning does two things: emails the new owner ("To-do assigned to you"), and records who did the reassigning on `assigned_by_staff_id` -- **distinct from `created_by_email`/`created_by_name`**, which is the original forwarder and never changes, while `assigned_by_staff_id` reflects the most recent manual reassignment via this picker. Assigning to the item's current owner (i.e. not actually changing anything) is a no-op -- no email, no bookkeeping change.
   - **Resolution notifications, back to whoever assigned it**: marking a reassigned to-do done, or discarding it, emails `assigned_by_staff_id` (if set) that it's resolved one way or the other. A to-do nobody manually reassigned (a self to-do, or one resolved purely by the inbound email's To/Cc routing) has no `assigned_by_staff_id` and so triggers no notification -- there's no one to report back to. Reopening a done item (`done: false`) does **not** notify -- only the transition to done, or an outright discard, counts as "resolved."
...
Every `staff.included` staff member gets a personal weekly email (**"Workflow Update"**); every included Partner additionally gets a combined firm-wide one (**"Overdue Summary"**). **These two now fire from two separate routes at two separate times** (split in v5.2 per the practice's requested schedule — see Settings → Notifications, and §6.8/§6.10 for the full picture including the timesheet reminders and daily digest): the Partner's Overdue Summary a morning ahead of everyone else's Workflow Update, not at the same moment. **Retimed in v5.4** from Sunday 20:00 AEST to **Sunday 12:00pm (midday) AEST** — the practice's actual requested time; "a night ahead" was the original (wrong) interpretation. Both authenticated via `CRON_SECRET` (the header Vercel Cron sends automatically once that env var is set).

**Still not actually delivering, as of a live production check on 4 Sep 2026** — two independent bugs in sequence, not yet both fixed:
1. The first full week live, `proxy.ts`'s matcher didn't exclude `/api/reports/*`, so NextAuth's middleware 307-redirected every cron request to `/login` before the route's own `CRON_SECRET` check ever ran. Fixed by adding `api/reports` to the matcher exclusion list (§9 Gotchas).
2. **That fix deployed, and every `/api/reports/*` cron now fires exactly on schedule — confirmed directly against Vercel's production runtime logs — but every single one comes back `401` from the route's own `authorize()` check**, e.g. `10:00:17 GET /api/reports/overdue-summary 401`. That check fails closed whenever `process.env.CRON_SECRET` is falsy, so `CRON_SECRET` is not actually correctly set for the **Production** environment in Vercel, despite an earlier assumption that it was — that assumption was based on the BAS approval emails sending successfully (§6.5), which are triggered by a button click and never check `CRON_SECRET` at all, so they proved nothing about this. **Action needed**: verify `CRON_SECRET` (and `RESEND_API_KEY`/`RESEND_FROM_EMAIL`) actually exist under Vercel → Project Settings → Environment Variables → Production specifically, not just Preview/Development. No redeploy should be needed once fixed — these are read at request time.

- **`GET /api/reports/overdue-summary`** — fires **Sunday 12:00pm AEST** (`0 2 * * 0` — AEST is UTC+10, no DST assumed since this practice is QLD-based). Sends `buildCombinedReportData`/`renderCombinedReportEmail` to every included Partner.
- **`GET /api/reports/monday-report`** — fires **Monday 07:00 AEST** (`0 21 * * 0`). Sends `buildStaffReportData`/`renderStaffReportEmail` to every included staff member.
...
- **Per-staff report (Workflow Update)**: a stand-out **"Prior week timesheet"** tile leads the top row (logged/standard hours for the week that just finished, green if a full week, amber if short, "—" if unavailable — no XPM link, XPM not configured, or the fetch failed; `fetchPriorWeekTimesheet`/`PriorWeekTimesheet` in `lib/mondayReport.ts`), then Overdue / Due this week / Due later / **Dashboard To-Dos** tiles, plus BAS/IAS-due and Payroll-due deadline tiles, a due-this-week task list, "Overdue, by client" grouped and sorted by count descending, and a **Dashboard To-Dos** list (added v5.4 — see below). **The task lists are deliberately uncapped** — every overdue task for every client, no "+N more" truncation. The size of the list is meant to be seen ("nobody should have this many items overdue" — Steve's own words), not hidden. One real consequence: a large backlog (one real case, 360 overdue tasks/26 clients) renders to ~250KB of HTML, over Gmail's ~102KB clip threshold — Gmail shows a "view entire message" link partway through; other clients don't clip at that size. Accepted trade-off, not a bug to fix.
- **Dashboard To-Dos, v5.4**: `StaffReportData.todoItems` (`TodoLine[]`, from `lib/todos.ts`'s new `listOutstandingTodoLines`) — outstanding email-forwarded To-Do items (§4.8) owned by this staff member, `pending_triage` or `todo` only (never `done`/`converted`, since neither needs more action). These live entirely outside the Task system, so without this they'd never appear anywhere but `/dashboard` itself. Same addition made to the Daily Digest (§6.10).
- **Every included Partner is CC'd on every individual's Workflow Update** (`app/api/reports/monday-report/route.ts`, via `getCombinedReportRecipients`) — so whoever's watching the practice sees each person's own report land, not just the separate firm-wide Overdue Summary. A Partner is excluded from the CC line on their own report.
- **Combined/Partner report (Overdue Summary)**: firm-wide totals, a **"Top overdue clients"** section (top 10 firm-wide by count, each with the single staff member holding the most of that client's overdue work), a per-staff mini-summary table, and an XPM-sourced timesheet summary (prior calendar week + FY-to-date logged hours per staff, via `computeWagesUtilisation`). Firm-wide totals are deduplicated across all tasks including unassigned ones, so they can be slightly higher than the sum of the per-staff mini-summary rows — intentional, not a bug.
- **Email HTML is table-based with inline styles only** — no CSS grid/flexbox/`@font-face`, since Outlook and many mobile clients strip anything fancier. This is a different, simpler visual language than the rest of the app (which is free to use flexbox etc.) — don't try to share components between the two. Shared with the timesheet reminder emails via `lib/emailTemplates/shared.ts` — see §6.8.
- `getAllTasks()` (`lib/workflow.ts`) — every task in the system, hydrated, unscoped by owner — feeds the combined report's firm-wide aggregation. Added specifically for this; also reused by `/bas-status` and `/team` (§6.7).
...
Three cron entry points (`vercel.json`), same `CRON_SECRET`-header auth and `isResendConfigured()` degrade-gracefully pattern as the Monday Report (§6.4) — Resend/`CRON_SECRET` are now configured (see §6.4), so these should also actually send once deployed. As with the Monday Report, "today"/"this week" are computed against Brisbane's wall clock (`aestNow`/`aestTodayIso`, reused directly from `lib/mondayReport.ts` rather than redefined) since the cron function itself runs in UTC. **Settings → Notifications** is the durable, human-facing record of the full six-email weekly schedule these two sections (§6.4 + §6.8) implement — check there first for "what fires when," this section is the implementation detail behind it.

- **`GET /api/reports/timesheet-reminder`** — **Timesheet Reminder #1**, fires **Monday 08:00 AEST** (`0 22 * * 0`). Sends every `included` staff member with an email (same recipient list as the Monday Report's individual reports, `getIndividualReportRecipients`) a plain "please submit your timesheet for last week" nudge — no hours data, just the ask, regardless of status.
- **`GET /api/reports/timesheet-reminder-2`** — **Timesheet Reminder #2**, fires **Monday 10:00 AEST** (`0 0 * * 1`). For the calendar week that just finished (Monday–Sunday, computed the same way the Monday Report's `buildTimesheetSummaries` finds its "prior week"), compares each XPM-linked staff member's logged hours against `standardHours` (38 for one person, one full week — §6.1's `STANDARD_HOURS_PER_DAY`) and sends a nudge (`renderFollowUpNudgeEmail`) **only to whoever's still short** — hours logged / standard / still short. Someone already at a full week by 10am doesn't get one.
- **`GET /api/reports/timesheet-followup`** — fires **Monday 12:00 AEST** (`0 2 * * 1`) and sends two distinct emails from the one trigger, reusing the same `buildFollowUpData()` week-status computation as Reminder #2 (a separate XPM timesheet fetch per route — an accepted duplicate cost of three independent cron triggers over the same underlying data):
  - **Timesheet Overview, to whoever's still short** (`buildPersonalShortfallData`/`renderPersonalShortfallEmail`) — Reminder #2 only tells someone about *this* week, so a gap from six weeks ago that never got backfilled would otherwise go unmentioned forever. This instead walks every completed week since the start of the current FY (`completedWeeksSince`) and lists every one still short (date range / logged / standard / short), plus a running total, plus that person's own FY-to-date **billable %** (`billableCapacityPct`) and **logged %** (`pct` — how much of their capacity is accounted for at all) so the reminder carries their own YTD context, not just the backlog. A person can receive both this and Reminder #2 in the same morning — they answer different questions ("am I short this week" vs. "what's my open backlog").
  - **Timesheet Summary, to every included Partner** (same recipients as the Monday Report's combined report, `getCombinedReportRecipients`) — sent regardless of whether anyone's short, so "everyone's logged a full week" is visible too, not just problems.
    - A **"Firm YTD billable %"** tile leads the top tiles row — the single firm-wide `billableCapacityPct` (§6.1's "one to performance-manage on" figure, the same one `lib/leaderboard.ts` weighs at 50% — §6.7) computed across every included **non-Partner** staff member at once (not an average of individual %s), colour-coded red/amber/green on the same thresholds as the Timesheets page's own indicator. **Was originally a per-employee "YTD billable overview" table** — dropped in favour of this one stand-out number once it was noticed the full per-employee breakdown already lives on `/timesheets` ("This FY") and would just be re-rendering that page; the email now points there in a one-line footer instead (`billableTone` in `lib/emailTemplates/timesheetReminders.ts`, `computeFirmFytdBillable` in `lib/timesheetReminders.ts`).
    - **"Last week — still incomplete"** table: the same set of people who got Reminder #2, with hours logged/standard/short.
...
## 6.10 Daily Digest — `lib/dailyDigest.ts`, `lib/emailTemplates/dailyDigest.ts`

A lighter, every-other-morning version of the Workflow Update (§6.4): just **overdue** and **due today**, per included staff member, via `GET /api/reports/daily-digest`. Same `CRON_SECRET`/`isResendConfigured()` pattern as everything else in §6.4/§6.8.

- **Fires Tue–Sun 7:00am AEST only** — `vercel.json`'s cron is `0 21 * * 1-6` (UTC Mon-Sat 21:00), which deliberately **excludes** the UTC-Sunday/AEST-Monday slot. Monday morning already gets the fuller Workflow Update, which covers overdue and due-today as part of its weekly report — a second email at the same moment would be a pure duplicate. The exclusion lives in the cron schedule itself, not a runtime day-of-week check, so there's nothing to misfire.
- **"Overdue"** is `dueDate < today`, **"due today"** is `dueDate === today`, both restricted to non-complete tasks off the staff member's own board (`getTasksForStaff` — owned + temporarily reassigned, same set `/my-work` shows). Deliberately uncapped, same convention as the Workflow Update's "Overdue, by client" section.
- Recipients are the same list as the Workflow Update's individual send (`getIndividualReportRecipients`, reused directly rather than redefined) — every `included` staff member with an email.
- No Partner CC on this one (unlike the Workflow Update, §6.4) — it's a lightweight daily nudge, not a report anyone else needs visibility into.
- **Dashboard To-Dos, v5.4**: same addition as the Workflow Update (§6.4) — a third tile plus a list section, via the same `listOutstandingTodoLines` helper (`DailyDigestData.todoItems`).

## 6.9 Settings → Notifications — `app/(dashboard)/settings/notifications/`, `lib/notificationRegistry.ts`

**Supersedes the old static "Email Schedule" page** (v5.6) — same schedule information, now interactive. `lib/notificationRegistry.ts`'s `NOTIFICATION_DEFINITIONS` is the single source of truth for both this page's listing and the preview route below, so the two can't drift apart on what exists. Lists all seven automated emails from §6.4/§6.8/§6.10 (category "staff"/"partner") plus the new draft client-facing one (§6.11, category "client") — audience/schedule/route/status per entry.

- **"View template"** opens `GET /api/admin/notifications/[key]/preview` (admin-gated) in a new tab — genuinely renders that notification's real template against **live current data** via each definition's `buildPreview()`, which just calls the same `build*`/`render*` functions the actual cron routes use. Deliberately not fabricated sample data: more useful to an admin ("what does this actually look like right now"), and can't drift out of sync with interface changes the way a parallel set of hand-written samples would. Staff-scoped previews use the first included staff member found (no "preview as" picker yet); the two "still short" ones (Timesheet Reminder #2, Timesheet Overview) throw a friendly "nothing to preview right now" message when nobody currently qualifies, shown inline rather than a raw error.
- **Recipients checklist is UI-only, not persisted** — there is no per-notification recipient-override storage in the data model; every staff-facing email still goes to the same `getIndividualReportRecipients()` pool and every Partner-facing one to `getCombinedReportRecipients()`, exactly as before. The checklist reflects that real pool (all checked by default) and toggling/saving is local `useState` only, with an explicit on-screen note and a "Save" button that shows "not saved" rather than silently pretending to persist. Built this way deliberately, to let the interaction be reviewed before committing to a schema for real overrides.
- Six concurrent cron jobs (§6.4/§6.8/§6.10) need to fit this project's Vercel plan limits (confirmed Pro, so this shouldn't bind). **As of 4 Sep 2026, none of them are actually delivering** — see §6.4's note on the `CRON_SECRET`-in-Production gap.

---
## 6.11 Client-facing notifications — draft, not built (v5.6)

Everything in §6.4/§6.8/§6.10/§6.9 emails the practice's own team (staff or Partners). This is the start of a second, genuinely different category: notifications sent to the **client themselves**. First one requested: **BAS Workflow**.

**What exists so far (all draft, nothing sends):**
- `lib/emailTemplates/basWorkflowClient.ts` — a template (`renderBasWorkflowClientEmail`), so "View template" on Settings → Notifications has something real to render. Sample data only (`{ clientName: "Acme Pty Ltd", ... }` in `lib/notificationRegistry.ts`) — see below for why it can't yet use real data.
- A **"Client notification"** checkbox on the task edit modal (`NewTaskModal.tsx`), shown only for BAS/IAS-typed tasks (`typeId === BAS_TASK_TYPE_ID`) — matches the "start with BAS Workflow" scope. When checked, a "fire on" picker: the task's own start date, a specific date, or daily/weekly/monthly/quarterly. **Local component state only** — not read by `handleSubmit`, nothing is persisted. Exists purely so the interaction could be reviewed before committing to a schema.
- Settings → Notifications lists it under a separate "Client Notifications" section (§6.9), badged "Draft".

**What's genuinely missing before any of this could fire for real** — flagged rather than guessed at:
1. **No client contact email anywhere in the data model.** `customers` only carries `partner_id`/`manager_id` (both *internal* staff allocations, §6) — there is no field for a client's own email address to send to. Needs a real decision: a new column on `customers`, a separate `customer_contacts` table (if a client can have more than one recipient), or something else — not decided yet.
2. **No schedule engine for the per-task trigger.** "On the task's start date" is a one-time computation off an existing column; "a specific date" needs a new column to hold it; "daily/weekly/monthly/quarterly" needs something to actually re-fire on a cadence *per task* — nothing like this exists (every cron in §6.4/§6.8/§6.10 is a single fixed practice-wide schedule, not one derived per-row from task data).
3. **No `tasks` columns for any of this** — `notification_enabled`, a trigger-type column, a custom-date column. Migration not yet written; holding off until (1) and (2) are resolved, since a half-designed schema here is worse than none.
4. **No recipient/consent model.** Emailing a client automatically raises questions this app has never had to answer for internal notifications — who at the client should get it, do they need to opt in, what if the BAS task's client has no contact on file yet.

Picking this back up: resolve (1)-(2) as real decisions (not code) first, then a migration, then wire `NewTaskModal`'s already-drafted UI to real fields, then a cron (or per-task scheduling mechanism) that actually sends `renderBasWorkflowClientEmail` with real task/client data instead of the sample in `lib/notificationRegistry.ts`.
...
- **`proxy.ts`'s matcher must explicitly exclude every route an external caller hits without a session cookie, or NextAuth's `authorized` callback 307-redirects it to `/login` before the route's own body ever runs.** Confirmed live: all five `/api/reports/*` cron routes (§6.4/§6.8) fired exactly on schedule for a full week per Vercel's runtime logs, every single one 307'd to `/login`, and **zero of the five report emails actually sent** despite `CRON_SECRET`/Resend being correctly configured — the routes' own `authorize()` Bearer-token check never even got a chance to run. Fixed by adding `api/reports` to the matcher's exclusion list (same pattern as the pre-existing `api/auth`/`api/email/inbound` exclusions). **Any future route meant for a cron job, webhook, or other non-browser caller needs the same exclusion added up front** — this class of bug is otherwise invisible: the cron fires "successfully" (Vercel logs a 307, not an error) and CI/build has no way to catch it.
...
- **Migration numbering is up to `025` as of v5.2** (`025_bas_stage_history_rls.sql`) — check `migrations/` for the actual latest before adding a new one; this file will lag reality.
- **`bas_stage_history` (migration 023) was created without RLS enabled** — an oversight, since every other business table (`003_workflow_schema.sql`) does enable it, with no policies (a deny-all for the anon/authenticated roles, since the app only ever reads/writes through the service-role key). Fixed in `025_bas_stage_history_rls.sql`. Worth double-checking any *future* new table migration includes the `ENABLE ROW LEVEL SECURITY` line — nothing currently enforces it.
...
- **Resend account/domain + `RESEND_API_KEY`/`RESEND_FROM_EMAIL`/`CRON_SECRET` are set in Vercel** — BAS Status approval emails (§6.5) are confirmed sending in production (landed in Junk initially, since resolved by marking not-junk). The Monday Report (§6.4) and timesheet reminder emails (§6.8) were **not** sending despite this — see §9's `proxy.ts` matcher gotcha, now fixed; confirm the next Sunday/Monday cycle actually lands before trusting it.
...
*Added in v5.2 (in review)*: the full six-email weekly schedule (§6.4/§6.8/§6.9) — Sunday-night Overdue Summary to the Partner, Monday-morning Workflow Update to everyone, two timesheet reminders through the morning, and a midday round of a Partner summary plus a personal whole-FY shortfall overview. Documented as a standing reference at Settings → Email Schedule, not just here.

*Closed in v5.2*: the BAS Status page (§6.5) is now self-gated server-side, same pattern as `/settings/karbon-import` — `page.tsx` checks `session.user.role === "admin"` itself and renders an "Admins only." notice instead of the board for non-admins, rather than relying on the nav link's admin-gate alone. Stage-change actions were already covered by the per-task `canModifyTask` check either way.
