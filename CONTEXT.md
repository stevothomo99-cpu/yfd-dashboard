...
Every `staff.included` staff member gets a personal weekly email (**"Workflow Update"**); every included Partner additionally gets a combined firm-wide one (**"Overdue Summary"**). **These two now fire from two separate routes at two separate times** (split in v5.2 per the practice's requested schedule — see Settings → Email Schedule, and §6.8/§6.10 for the full picture including the timesheet reminders and daily digest): the Partner's Overdue Summary a morning ahead of everyone else's Workflow Update, not at the same moment. **Retimed in v5.4** from Sunday 20:00 AEST to **Sunday 12:00pm (midday) AEST** — the practice's actual requested time; "a night ahead" was the original (wrong) interpretation. Both authenticated via `CRON_SECRET` (the header Vercel Cron sends automatically once that env var is set).
...
## 6.9 Settings → Email Schedule — `app/(dashboard)/settings/email-schedule/`

A static, admin-only notes page (self-gated the same way `/settings/karbon-import` is, since it mutates nothing and has no API route of its own to separately enforce admin on) recording the practice's requested weekly email schedule as a durable reference inside the app itself, not just in this file. Lists all seven automated emails from §6.4/§6.8/§6.10 in firing order with audience/time/content, plus short explanatory notes on the reasoning CONTEXT.md itself gives above (why two nudges before midday, why the Overdue Summary fires the morning before, why the Daily Digest skips Monday). **This is the schedule as of v5.4** — six Vercel Cron jobs total:

| # | Email | Audience | When (AEST) | Route |
|---|---|---|---|---|
| 1 | Overdue Summary | Partner | Sun 12:00pm | `/api/reports/overdue-summary` |
| 2 | Workflow Update | Each employee | Mon 7:00am | `/api/reports/monday-report` |
| 3 | Timesheet Reminder #1 | Each employee | Mon 8:00am | `/api/reports/timesheet-reminder` |
| 4 | Timesheet Reminder #2 | Employees still short | Mon 10:00am | `/api/reports/timesheet-reminder-2` |
| 5 | Timesheet Summary | Partner | Mon 12:00pm | `/api/reports/timesheet-followup` |
| 6 | Timesheet Overview | Employees still short | Mon 12:00pm | `/api/reports/timesheet-followup` |
| 7 | Daily Digest | Each employee | Tue–Sun 7:00am | `/api/reports/daily-digest` |

Six concurrent cron jobs need to fit this project's Vercel plan limits (confirmed Pro, so this shouldn't bind) — check that if fewer emails run than expected. **As of 4 Sep 2026, none of these are actually delivering** — see §6.4's note on the `CRON_SECRET`-in-Production gap.
