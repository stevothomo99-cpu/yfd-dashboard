...
- **`GET /api/reports/timesheet-followup`** — fires **Monday 12:00 AEST** (`0 2 * * 1`) and sends three distinct emails, all from the one trigger. For the calendar week that just finished (Monday–Sunday, computed the same way the Monday Report's `buildTimesheetSummaries` finds its "prior week"):
  ...
  - **Last-week nudge, to whoever's still short**: states hours logged / standard / still short for the week that just finished. Someone already at a full week doesn't get one.
  - **Whole-FY shortfall email, also to whoever's short** (`buildPersonalShortfallData`/`renderPersonalShortfallEmail`) — the last-week nudge only tells someone about *this* week, so a gap from six weeks ago that never got backfilled would otherwise go unmentioned forever. This instead walks every completed week since the start of the current FY (`completedWeeksSince`) and lists every one still short (date range / logged / standard / short), plus a running total, plus that person's own FY-to-date **billable %** (`billableCapacityPct`) and **logged %** (`pct` — how much of their capacity is accounted for at all) so the reminder carries their own YTD context, not just the backlog. A person can receive both this and the last-week nudge in the same run — they answer different questions ("am I short this week" vs. "what's my open backlog").
  - **Every included Partner always gets a firm-wide summary email** (same recipients as the Monday Report's combined report, `getCombinedReportRecipients`) — sent regardless of whether anyone's short, so "everyone's logged a full week" is visible too, not just problems. Two tables:
    - **"Last week — still incomplete"**: the same set of people who got a last-week nudge, with hours logged/standard/short.
