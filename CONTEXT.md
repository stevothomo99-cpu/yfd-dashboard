...
Every `staff.included` staff member gets a personal weekly email (**"Workflow Update"**); every included Partner additionally gets a combined firm-wide one (**"Overdue Summary"**). **These two now fire from two separate routes at two separate times** (split in v5.2 per the practice's requested schedule — see Settings → Email Schedule, and §6.8/§6.10 for the full picture including the timesheet reminders and daily digest): the Partner's Overdue Summary a night ahead of everyone else's Workflow Update, not at the same moment. Both authenticated via `CRON_SECRET` (the header Vercel Cron sends automatically once that env var is set).
...
- **`GET /api/reports/overdue-summary`** — fires **Sunday 20:00 AEST** (`0 10 * * 0` — AEST is UTC+10, no DST assumed since this practice is QLD-based). Sends `buildCombinedReportData`/`renderCombinedReportEmail` to every included Partner.
