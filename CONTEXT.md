...
- **`GET /api/reports/timesheet-followup`** — fires **Monday 12:00 AEST** (`0 2 * * 1`) and sends two distinct emails from the one trigger, reusing the same `buildFollowUpData()` week-status computation as Reminder #2 (a separate XPM timesheet fetch per route — an accepted duplicate cost of three independent cron triggers over the same underlying data):
  ...
  - **Timesheet Summary, to every included Partner** (same recipients as the Monday Report's combined report, `getCombinedReportRecipients`) — sent regardless of whether anyone's short, so "everyone's logged a full week" is visible too, not just problems. Two tables:
    - **"Last week — still incomplete"**: the same set of people who got Reminder #2, with hours logged/standard/short.
