...
Every `staff.included` staff member gets a personal weekly email (**"Workflow Update"**); every included Partner additionally gets a combined firm-wide one (**"Overdue Summary"**). **These two now fire from two separate routes at two separate times** (split in v5.2 per the practice's requested schedule — see Settings → Email Schedule, and §6.8 for the full picture including the timesheet reminders): the Partner's Overdue Summary a night ahead of everyone else's Workflow Update, not at the same moment. Both authenticated via `CRON_SECRET` (the header Vercel Cron sends automatically once that env var is set). `RESEND_API_KEY`/`RESEND_FROM_EMAIL`/`CRON_SECRET` are set in Vercel (same Resend account already confirmed sending the BAS approval emails, §6.5) — but **the first full week live, none of the five `/api/reports/*` crons actually sent anything**, despite firing exactly on schedule: `proxy.ts`'s matcher didn't exclude `/api/reports/*`, so NextAuth's middleware 307-redirected every cron request to `/login` before the route's own `CRON_SECRET` check ever ran (§9 Gotchas). Fixed by adding `api/reports` to the matcher exclusion list — confirm the next Sunday/Monday cycle actually lands before trusting this fully.
...
A static, admin-only notes page (self-gated the same way `/settings/karbon-import` is, since it mutates nothing and has no API route of its own to separately enforce admin on) recording the practice's requested weekly email schedule as a durable reference inside the app itself, not just in this file. Lists all six automated emails from §6.4/§6.8 in firing order with audience/time/content, plus short explanatory notes on the reasoning CONTEXT.md itself gives above (why two nudges before midday, why the Overdue Summary fires the night before). **This is the schedule as of v5.2** — five Vercel Cron jobs total:
...
Five concurrent cron jobs need to fit this project's Vercel plan limits — check that if fewer emails run than expected once real Resend/Vercel wiring is live.
